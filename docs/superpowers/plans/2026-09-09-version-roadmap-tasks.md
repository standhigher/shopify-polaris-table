...
- 当前发布基线为 `v1.0.0`；0.5 的 V1 稳定性工作与 0.6.x 的部分能力已完成。
...
| `0.8` | POC 已发布（`v0.8.0`） | 高级交互与大数据量 | 公开 API 仍需真实场景与性能证据 |
| `1.0` | 已发布（`v1.0.0`） | API 稳定性基线 | 多产品接入验证作为 1.x 持续验证 |
...
- [x] 固定仅支持包根与 Extension 官方子入口，并以 npm 包消费者测试阻止 `src/experimental` 进入 tarball。
...
- [x] 为 `ExtensionTable` 增加不加载 Admin renderer 的官方子入口，并以无 Polaris 的包消费者测试确认 Admin 与 Extension 不共享不必要的 UI 依赖。
...
**发布后跟进：** 真实接入证据、公共 API 兼容性和迁移路径仍需在 1.x 持续验证；只有具备证据的能力才能从 preview 提升为稳定 API。
